          ...
          child: Padding(
            ...
            child: ListView(
              children: [
                ...
                Row(
                  children: [
                    Text("${model.data.price.toString()} EGP",style: TextStyle(fontSize: 20,color: Colors.teal),),
                    Spacer(),
                    CircleAvatar(
                      child: IconButton(icon: Icon(Icons.add_shopping_cart_sharp,color: Colors.teal,),onPressed: (){
                      HomeLayoutCubit.get(context).changeCart(id: model.data.id);
                    },),
                      backgroundColor:(HomeLayoutCubit.get(context).isCart[id]??true)?Colors.teal: Colors.grey[200],minRadius: 26,
                    ),
                  ],
                ),
                Padding(
                  padding: const EdgeInsets.all(20.0),
                  child: Divider(height: 2,color: Colors.teal,endIndent: 10,indent: 10,),
                ),
                SizedBox(height: 15,),
