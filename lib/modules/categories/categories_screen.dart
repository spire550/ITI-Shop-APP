...
import 'package:my_shop/modules/categoryDetails/category_details.dart';
class Categories extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    child: BlocConsumer<HomeLayoutCubit,HomeLayoutStates>(
        builder:(context,state)=> HomeLayoutCubit.get(context).homeModel == null || HomeLayoutCubit.get(context).categoriesModel == null? Center(child: CircularProgressIndicator(color: Colors.teal,)) : ListView.separated(physics: BouncingScrollPhysics(),itemBuilder: (context,index)=>categoriesView(HomeLayoutCubit.get(context).categoriesModel,index,context), separatorBuilder: (context,index)=>Divider(height: 2,color: Colors.teal,indent: 25,endIndent: 25,), itemCount: HomeLayoutCubit.get(context).categoriesModel!.dataAll.dataList.length),
    ...
    child: Row(
      children: [
        ...
        Padding(
          ...
          child: Text(model.dataAll.dataList[index].name,style: TextStyle(color: Colors.teal,fontSize: 17,fontWeight: FontWeight.bold)),
        ),
        Spacer(),
        IconButton(
            onPressed: (){
